...
# 로직 2 : image binarization(이미지 이진화)
...
# 참고 블로그 : https://deep-learning-study.tistory.com/245
...
def non_maximum_supression(bboxes, threshold=0.5):
    ...
    for _, bbox in enumerate(bboxes):
        ...
        for new_bbox in new_bboxes:
            ...
            # 로직 4 : 두 bbox의 겹치는 영역을 구해서, 영역이 안 겹칠때 new_bbox로 save
            """
            x_overlap = min(x1_br,x2_br) - max(x1_tl, x2_tl) if x1_tl <= x2_tl <= x1_br or x1_tl <= x2_br <= x1_br else 0 
            y_overlap = min(y1_br,y2_br) - max(y1_tl, y2_tl) if y1_tl <= y2_tl <= y1_br or y1_tl <= y2_br <= y1_br else 0
...
class HumanDetector(Node):
    ...
    def __init__(self):
        ...
        # HoG(Histogram of Gradinents)를 수행하는 Descriptor정의
        # 미리 학습된 데이터가 OpenCV 내부에 저장되어 있다.
        self.pedes_detector = cv2.HOGDescriptor()

        # SVM(Support Vector Machine)을 수행하는 descriptor 
        # cv2.HOGDescriptor_getDefaultPeopleDetector() 미리 훈련된 특징 벡터를 가져오는구문                            
    ...
    def detect_human(self, img_bgr):
        ...
        # 로직 3 : human detection 실행 후 bounding box 출력
        # detectMultiScale() HOG 멀티스케일 객체 검출 함수 -> 입력 영상을 조금씩 축소하면서 다양한 크기에 대해 검출
        # 출력값은 foundlocations 사각형 영역 정보
        ...
        if len(rects_temp) != 0:
            ...
            if self.able_to_pub:
                ...
                obj_list = [int(_) for _ in range(len(rects))]
