...
class a_star(Node):
    ...
    def grid_cell_to_pose(self,grid_cell):
        ...
        x = self.map_resolution * grid_cell[0] + self.map_offset_x
        y = self.map_resolution * grid_cell[1] + self.map_offset_y
    ...
    def goal_callback(self,msg):
        ...
        # if msg.header.frame_id=='map':
            ...
            self.goal = goal_cell
            # pose_to_grid_cell 함수를 사용해서 cell로 바꿔줘라
            print(msg)
    ...
    def dijkstra(self,start):
        Q = deque()                         # 큐 선언
        Q.append(start)                     # 시작지 좌표 append(add)  map_point 좌표
        ...
        while ?? 
            ...
            node = ??
        # '''
        while Q:    # 큐가 empty일 때 까지
            if found:  # 목표위치를 찾으면 빠져나온다.
                break

            current = Q.popleft()       # Q의 popleft()를 이용해 탐색할 노드 선택

            for i in range(8):
                next = [current[0] + self.dx[i], current[1] + self.dy[i]]     # current에 인접한 노드 선택(dx, dy를 이용해 next가 for문 돌 때마다 바뀌게 설정)
                if next[0] >= 0 and next[1] >= 0 and next[0] < self.GRIDSIZE and next[1] < self.GRIDSIZE:   # 범위 안
                        if self.grid[next[0]][next[1]] < 50:        # next의 grid값이 50보다 작으면 로봇이 갈 수 있다는 의미
                            if self.cost[current[0]][current[1]] + self.dCost[i] < self.cost[next[0]][next[1]] and self.path[next[0]][next[1]] == 0:     # 코스트 계산 -> 코스트가 낮으면 path, cost 변수를 갱신하고 Q에 next를 넣음
                                Q.append(next)
