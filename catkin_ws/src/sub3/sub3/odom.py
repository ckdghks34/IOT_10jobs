...
class odom(Node):
    ...
    def __init__(self):
        ...
        # self.x=self.turtlebot_status_msg.twist.angular.x
        self.x=0.0
        # self.y=self.turtlebot_status_msg.twist.angular.y
        self.y=0.0
        # self.theta=self.turtlebot_status_msg.twist.linear.z * pi/180
        self.theta=0.0
    ...
    def imu_callback(self,msg):
        ...
        if self.is_imu ==False :    
            self.is_imu=True
            imu_q = Quaternion(0.1 , 0, 0, -1)
            self.imu_offset = 0
        else :
            imu_q= Quaternion(msg.orientation.w , msg.orientation.x, msg.orientation.y, msg.orientation.z)
            # quaternion을 euler로 바꾼후 각도를 radian값으로 변경
            self.theta= imu_q.to_euler()[2]
